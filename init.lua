...
-- Line spacing (in pixels for GUI, has no effect in terminal)
...
vim.opt.foldlevel = 1 -- start with some folds closed
vim.opt.foldlevelstart = 99 -- but open all on file open
...
require('lazy').setup({
  ...
  { 'Bilal2453/luvit-meta', lazy = true },
  {
    ...
    config = function()
      ...
      local servers = {
        clangd = {
          cmd = { 'clangd', '--background-index', '--clang-tidy' }, -- Optional: Add extra flags
          filetypes = { 'c', 'cpp', 'objc', 'objcpp' }, -- File types clangd should handle
          root_dir = require('lspconfig.util').root_pattern('.clangd', 'compile_commands.json', '.git'), -- Project root
  ...
  { -- Highlight, edit, and navigate code
    ...
    dependencies = {
      -- Shows context of the current function/class at the top of the buffer
    ...
    opts = {
      ...
      auto_install = false,
      -- Force sync install on startup to prevent repeated installations
      sync_install = false,
      -- Specify parser install directory
